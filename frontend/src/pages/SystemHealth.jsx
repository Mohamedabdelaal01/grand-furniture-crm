  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-dark-400 text-xs font-bold">{label}</span>
      </div>
      <div className="text-white text-2xl font-black">{value}</div>
      {sub && <div className="text-dark-500 text-[11px] mt-1">{sub}</div>}
    </div>
  );
}
...
export default function SystemHealth() {
  ...
  if (!h) {
    return <div className="card p-12 text-center text-dark-400 font-bold" dir="rtl">تعذّر تحميل بيانات النظام</div>;
  }
  ...
  return (
    <div className="max-w-[1100px] mx-auto space-y-6 pb-12" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-primary-500 font-black text-[10px] uppercase tracking-[0.2em]">البنية التحتية</span>
          </div>
          <h1 className="text-3xl font-black text-white flex items-center gap-2">
            <Activity className="w-7 h-7 text-primary-400" />
            صحة النظام
          </h1>
          <p className="text-dark-400 text-sm mt-1">راقب حجم قاعدة البيانات والذاكرة قبل ما تسبب أي توقف مفاجئ.</p>
        </div>
      ...
      <div className={`card p-4 flex items-center gap-3 ${healthy ? 'border-emerald-500/30' : 'border-amber-500/40'}`}>
        ...
        <div>
          <div className="text-white font-black">{healthy ? 'النظام يعمل بشكل سليم' : 'انتبه — مؤشرات تحتاج مراجعة'}</div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Metric icon={Server} tone="bg-primary-500/15 text-primary-400"
      ...
      <div className="card overflow-hidden">
        <div className="p-4 flex items-center gap-2 border-b border-dark-800">
          ...
          <h3 className="text-white font-black text-sm">عدد الصفوف في كل جدول</h3>
        </div>
        <div className="p-4 space-y-2.5">
          {Object.entries(h.tables || {}).map(([t, n]) => (
            <div key={t}>
              <div className="flex items-center justify-between text-xs mb-1">
                <span className="text-dark-200 font-bold">{TABLE_AR[t] || t}</span>
                <span className="text-white font-black">{n == null ? '—' : fmt(n)}</span>
              </div>
              <div className="h-1.5 rounded-full bg-dark-800 overflow-hidden">
                <div className="h-full rounded-full bg-primary-600/70"
      ...
      <p className="text-dark-600 text-[11px] text-center">يتحدّث تلقائياً كل ١٥ ثانية</p>
    </div>
  );
}
