...
const TaskRow = ({ task, onDone, onDelete, navigate }) => {
  ...
  const tone = overdue
    ...
    : isToday
      ...
      : { bd: 'border-border', bg: 'bg-surface-secondary/40', dot: 'text-muted', Icon: Clock };
  ...
  return (
    <div className={`flex items-start gap-3 p-4 rounded-xl border ${tone.bd} ${tone.bg}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {task.note && <p className="text-foreground text-xs mt-1 leading-relaxed">{task.note}</p>}
        <div className="flex items-center gap-2 mt-2 flex-wrap">
          <span className={`text-[11px] font-bold ${tone.dot}`}>
            {overdue ? 'متأخرة • ' : isToday ? 'النهاردة • ' : ''}{task.due_at}
          </span>
          {task.source === 'reschedule' && (
            <span className="text-[10px] bg-surface-secondary text-muted px-2 py-0.5 rounded-full">من مكالمة</span>
          )}
        </div>
      </div>
  ...
  if (error) {
    return (
      <div className="card p-10 text-center border-rose-500/20 bg-rose-500/5">
        ...
        <p className="text-foreground font-black mb-1">تعذّر تحميل المهام</p>
  ...
  if (tasks.length === 0) {
    return (
      <div className="card p-16 text-center">
        ...
        <p className="text-muted font-bold">مفيش مهام مفتوحة</p>
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-foreground font-black">مهامي ({tasks.length})</h3>
      </div>
