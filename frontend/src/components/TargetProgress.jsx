...
export default function TargetProgress({ target = 0, percent = 0, label }) {
  ...
  if (!target || target <= 0) {
    return (
      <p className="text-muted text-[11px] mt-3 font-bold">— لم يتم تحديد مستهدف بعد —</p>
    );
  }

  const pct = Math.max(0, Math.round(percent || 0));
  const bar = pct >= 100 ? 'from-emerald-500 to-emerald-400'
    : pct >= 60          ? 'from-accent to-accent'
  ...
  return (
    <div className="mt-3 space-y-1.5">
      <div className="flex items-center justify-between text-[11px] font-bold gap-2">
        <span className="text-muted">{effectiveLabel}: {fmt(target)} تعاقد</span>
        <span className={pct >= 100 ? 'text-emerald-400' : 'text-foreground'}>
          تم تحقيق: {pct}%
        </span>
      </div>
