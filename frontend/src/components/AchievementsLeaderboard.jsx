...
const Bar = ({ value, color = 'primary' }) => {
  ...
  return (
    <div className="h-1.5 bg-dark-800 rounded-full overflow-hidden w-20">
      <div
...
const AchievementsLeaderboard = ({ rows = [], entityType = 'sales', weights, loading }) => {
  ...
  if (!rows.length) {
    return (
      <div className="card p-12 text-center">
        ...
        <p className="text-dark-400 font-bold">لسه مفيش بيانات إنجازات</p>
  ...
  return (
    <div className="card overflow-hidden">
      {weights && (
        <div className="px-4 py-2.5 border-b border-dark-800 text-[11px] text-dark-400 flex items-center gap-3 flex-wrap">
          <span className="font-bold text-dark-300">معادلة النقاط:</span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-primary-500" />
            متابعة <b className="text-primary-400">{weights.followup}%</b>
          </span>
          <span className="flex items-center gap-1">
        ...
        <table className="w-full text-xs whitespace-nowrap">
          <thead>
            <tr className="bg-dark-800/60 text-dark-400 text-right font-black uppercase tracking-wider">
              ...
              {isSales && <th className="py-3 px-3 text-dark-500">الفرع</th>}
              ...
              <th className="py-3 px-3 text-center bg-primary-500/5">النقاط</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr
                  key={`${id}-${r.branch || ''}`}
                  className={`border-t border-dark-800/60 hover:bg-dark-800/20 transition-colors ${
                ...
                >
                  <td className="py-3 px-3 text-center">
                    {rankStyle ? (
                      ...
                      <span className="text-dark-500 font-bold">#{r.rank}</span>
                    )}
                  </td>
                  <td className="py-3 px-3 text-white font-black">{id}</td>
                  {isSales && <td className="py-3 px-3 text-dark-400">{r.branch || '—'}</td>}
                  <td className="py-3 px-3 text-center text-dark-200 font-bold">{r.phones_received}</td>
                  <td className="py-3 px-3 text-center text-primary-400 font-bold">{r.followups_done}</td>
                  ...
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-primary-400 font-black text-[11px] w-9">{r.followup_rate}%</span>
                    </div>
                  </td>
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-2">
                  ...
                  <td className="py-3 px-3 text-center bg-primary-500/5">
      ...
      <div className="px-4 py-3 border-t border-dark-800 text-[11px] text-dark-500 leading-relaxed">
